      ...
      <Table ref="table" v-loading="loading" :page.sync="currentPage" :size.sync="pageSize" :has-page="true" @row-click="handleTableRowClick">
          ...
          <template slot-scope="scope">
            ...
            <el-table-column label="累计充值额" align="center">
              <template slot-scope="scope">
                <span>￥{{ scope.row.echargeAmrount }}</span>
              </template>
            </el-table-column>
...
<script>
  import { getPlatformCarAccountList,exportTemp} from '@/api/fleetAccount'
...
export default {
  ...
  methods: {
    ...
    handleTableRowClick(row) {
      ...
      this.$router.push({name:'fleetListDetails',params:{value:this.currentEditParams}})
    },
    // 编辑界面
    editButton(row){
      this.currentEditParams = { ...row }

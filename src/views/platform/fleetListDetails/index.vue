<template>
  <div class="app-container">
    <!-- 标题 -->
    <h3 class="title">
      <el-button @click="$router.push('/account/fleetAccountOfPlatform')" type="text" icon="el-icon-back" ghost>返回</el-button>
      ...
      <el-tabs v-model="activeName" @tab-click="handleClick">
        ...
        <el-tab-pane label="还款流水" name="3">
          ...
          <Table ref="hk" v-loading="loadingHk" :page.sync="currentPagehk" :size.sync="pageSize" :has-page="true">
            ...
            <el-table-column label="欠款方" align="center">
              <template slot-scope="scope">
            ...
            <el-table-column label="打款时间" align="center">
              <template slot-scope="scope">
                <span>{{ scope.row.paymentTime!==null?moment(scope.row.paymentTime).format('YYYY-MM-DD'):'-' }}</span>
              </template>
            </el-table-column>
...
<script>
  import { getRechargeList,getRechargeCard,getConsumptionList,getConsumptionCard,getHkList,getHkCard,addHKAccountList,exportInTemp,exportOutTemp,exportHkTemp } from '@/api/fleetAccount'
...
export default {
  ...
  mounted() {
    if(this.$route.params.value===undefined){
      this.$router.push('/account/fleetAccountOfPlatform')
    }else {

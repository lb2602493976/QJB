<template>
  <div class="app-container">
    <!-- 标题 -->
    <h3 class="title">
      <el-button @click="$router.push('/account/platformAccountRecharge')" type="text" icon="el-icon-back" ghost>返回</el-button>
...
<script>
  import { getRechargeList,getRechargeCard,getConsumptionList,getConsumptionCard,getHkList,getHkCard,addHKAccountList,exportInTemp,exportOutTemp,exportHkTemp } from '@/api/ptAccount'
...
export default {
  ...
  mounted() {
    if(this.$route.params.value===undefined){
      this.$router.push('/account/platformAccountRecharge')
    }else {

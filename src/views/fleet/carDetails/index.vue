<template>
  <div class="app-container">
    <!-- 标题 -->
    <h3 class="title">
      <el-button @click="$router.push('/account/fleetAccountRecharge')" type="text" icon="el-icon-back" ghost>返回</el-button>
    ...
    <div class="main">
      <div class="header">
        <div>
          ...
          <el-select clearable placeholder="审核状态"  v-model="status" size="small" style="width: 330px;margin-left: 10px" v-if="activeName!=='1'">
            <el-option label="待确认" value="0"/>
            <el-option label="审核通过" value="1"/>
            <el-option label="审核拒绝" value="2"/>
          </el-select>
          <el-select v-model.trim="carIds" clearable filterable multiple placeholder="请选择车牌号" style="width: 330px;margin-left: 10px" v-else>
            <el-option
              v-for="item in cars"
              :key="item.carId"
              :label="item.carNum"
        ...
        <el-tab-pane label='消费流水' name="1">
          ...
          <Table ref="out" v-loading="loadingOut" :page.sync="currentPageout" :size.sync="pageSize" :has-page="true">
            ...
            <el-table-column label="支付方式" align="center">
              <template slot-scope="scope">
                <span>{{ scope.row.payType === '1' ? '线上' : scope.row.payType === '2' ? '预付费' : '收银台' }}</span>
              </template>
            </el-table-column>
...
<script>
  import { getRechargeList,getRechargeCard,getConsumptionList,getConsumptionCard,getHkList,getHkCard,addHKAccountList,exportInTemp,exportOutTemp,exportHkTemp } from '@/api/carAccount'
...
export default {
  ...
  mounted() {
    ...
    if(this.$route.params.value===undefined){
      this.$router.push('/account/fleetAccountRecharge')
    }else {
  ...
  methods: {
    ...
    getFormatedSearchParams() {
      ...
      if(this.activeName ==='1'){
        ...
        query.append('carId', this.carIds)
        // carId:this.carIds.toString(),
      }else if(this.activeName ==='2'){
    ...
    hk(){
      this.currentEditParams ={
        ...
        name:this.fleetList.name,
        code:this.fleetList.code
      }
...
// 查询所有
    ...
    handleEditHkConfirm(params){
      params.rechargeType = params.online
    ...
    handleExportTemp(){
      ...
      if(this.activeName==='1'){
        ...
        return exportOutTemp(parms).then( response =>{
          download(response.data,'车队消费统计.xlsx')
        ...
        return exportInTemp(parms).then( response =>{
          download(response.data,'车队充值统计.xlsx')
        }).catch(e=>console.log(e))
      }else if(this.activeName==='3'){
        if(this.value2!=='' && this.value2!==null){
          parms = {
        ...
        return exportHkTemp(parms).then( response =>{
          download(response.data,'车队还款统计.xlsx')
        }).catch(e=>console.log(e))
      }

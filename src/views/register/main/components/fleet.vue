<template>
  <el-form :ref="formName" :model="params" :rules="rules" :size="size" style="text-align:left;" :label-position="labelPosition"
    @keyup.enter.native="handleConfirm">
        <el-form-item label="用户类型" :label-width="formLabelWidth" required>
          <el-radio-group v-model.trim="params.natureType" @change="handleRadioChange">
            <el-radio label="1" name="1">公司</el-radio>
            <el-radio label="0" name="0">个人</el-radio>
          </el-radio-group>
        </el-form-item>
        <el-form-item label="用户名" prop="userName" :label-width="formLabelWidth">
          <el-input v-model.trim="params.userName" placeholder="请输入" />
        </el-form-item>
        <el-form-item v-if="params.natureType==='1'" label="企业代码" prop="enterpriseCode" :label-width="formLabelWidth">
          <el-input v-model.trim="params.enterpriseCode" placeholder="请输入" />
        </el-form-item>
        <el-form-item v-else-if="params.natureType==='0'" label="身份证号" prop="idCard" :label-width="formLabelWidth">
        ...
        <el-form-item label="手机验证码" prop="verifyCode" :label-width="formLabelWidth">
          <div class="verify-code-container">
            ...
            <el-button @click="handleGetVerifyCode" :disabled="countDown!==0" :loading="verifyCodeLoading">
              获取验证码<span v-show="countDown!==0">（{{ countDown }}）s</span>
            </el-button>
          </div>
        </el-form-item>
        <!-- <label class="el-form-item__label"
  ...
  export default {
    data() {
      ...
      return {
        ...
        params: {
          ...
          remark: '',
          certList:[
            { certName:'道路运输经营许可证（经营许可证）照片',certCode: '',certPc: [], },
          ],
          idCard:'',
        },
        rules: {
    ...
    methods: {
      ...
      getParams() {
        ...
        if(params.natureType==='0') delete params.enterpriseCode
        if(params.natureType==='1') delete params.idCard
        return {
          ...params,
          parentTenantId: getTenant(),
          tenantType: '2',
          certList:params.certList.map(item => {
            console.log(item.certPc)
            return {
              ...item,
      ...
      handleRadioChange(val){
        this.params.certList = [
          [{ certName:'身份证正反面照片',certCode: '',certPc: [], },],
          [{ certName:'道路运输经营许可证（经营许可证）照片',certCode: '',certPc: [], },]
        ][val]
      },
    }

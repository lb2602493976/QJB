  ...
  <div class="app-container">
    ...
    <div class="main">
      ...
      <Table v-loading="loading" @row-click="handleTableRowClick" ref="table" :page.sync="currentPage" :size.sync="pageSize" :hasPage="true">
        ...
        <el-table-column label="报（检）修时间" width="150" align="center">
          <template slot-scope="scope">
            <span>{{ scope.row.repairTime }}</span>
          </template>
        </el-table-column>

        <el-table-column label="接报人" width="160" align="center">
          <template slot-scope="scope">
            <span>{{ scope.row.criminalPeople }}</span>
          </template>
        </el-table-column>

        <el-table-column label="故障原因" width="160" align="center">
          <template slot-scope="scope">
            <span>{{ scope.row.faultCause }}</span>
          </template>
        </el-table-column>

        <el-table-column label="原因及检修结果" width="160" align="center">
          <template slot-scope="scope">
            <span>{{ scope.row.faultResults }}</span>
          </template>
        </el-table-column>

        <el-table-column label="检修人员" align="center">
          <template slot-scope="scope">
            <span>{{ scope.row.maintenancePeople }}</span>
          </template>
        </el-table-column>

        <el-table-column label="检修时间" align="center">
          <template slot-scope="scope">
...
<script>
import { searchRecord,createRecord,updateRecord,deleteRecord } from '@/api/overhaulEquipment'

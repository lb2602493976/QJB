  ...
  <div class="app-container">
    ...
    <div class="main">
      ...
      <Table v-loading="loading" @row-click="handleTableRowClick" ref="table" :page.sync="currentPage" :size.sync="pageSize" :hasPage="true">
        ...
        <el-table-column label="单位名称" width="150" align="center">
          <template slot-scope="scope">
            <span>{{ scope.row.companyName }}</span>
          </template>
        </el-table-column>

        <el-table-column label="演习参加人员" width="160" align="center">
          <template slot-scope="scope">
            <span>{{ scope.row.participants }}</span>
          </template>
        </el-table-column>

        <el-table-column label="演习开始时间" width="160" align="center">
          <template slot-scope="scope">
            <span>{{ scope.row.startTime }}</span>
          </template>
        </el-table-column>

        <el-table-column label="演习结束时间" width="160" align="center">
          <template slot-scope="scope">
            <span>{{ scope.row.endTime }}</span>
          </template>
        </el-table-column>

        <el-table-column label="演习内容及步骤" width="160" align="center">
          <template slot-scope="scope">
            <span>{{ scope.row.exerciseContent }}</span>
          </template>
        </el-table-column>

        <el-table-column label="演习效果评价及完善措施" width="300" align="center">
          <template slot-scope="scope">
            <span>{{ scope.row.exerciseEffect }}</span>
          </template>
        </el-table-column>

        <el-table-column label="加气站长" width="160" align="center">
          <template slot-scope="scope">
            <span>{{ scope.row.lpgStations }}</span>
          </template>
        </el-table-column>

        <el-table-column label="公司安全部门" width="160" align="center">
          <template slot-scope="scope">
            <span>{{ scope.row.securityDept }}</span>
          </template>
        </el-table-column>

        <el-table-column label="公司经理" width="160" align="center">
          <template slot-scope="scope">
            <span>{{ scope.row.companyManager }}</span>
          </template>
        </el-table-column>
...
<script>
import { searchRecord,createRecord,updateRecord,deleteRecord } from '@/api/emergencyRescueDrill'

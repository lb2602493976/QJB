      ...
      <el-col :xs="24" :sm="24" :md="20" :lg="20" :xl="20">
        ...
        <el-table v-loading="loading" :data="tableData" border style="width: 100%">
          ...
          <el-table-column label="岗位" align="center">
            <template slot-scope="scope">
              <div>{{ jobTypeFilter(scope.row.jobType)}}</div>
            </template>
          </el-table-column>
          ...
          <el-table-column label="状态" align="center">
            <template slot-scope="scope">
              ...
              <div v-for="item in statusList" :key="item.id">
                <el-tag v-if="scope.row.lockFlag.toString() == item.itemValue" :type="scope.row.lockFlag=='0' ? '' : 'info'">{{
                  item.itemText }}
                </el-tag>
              </div>
            </template>
          ...
          <el-table-column label="操作" fixed="right" min-width="150" align="center">
            <template slot-scope="scope">
              ...
              <el-button v-show="scope.row.status!='1'" size="small" icon="el-icon-delete" type="danger" @click="handleDelete(scope.row)">删除
              </el-button>
            </template>
          </el-table-column>
        </el-table>
        ...
        <el-dialog :title="operation?'新增用户':'编辑用户'" :visible.sync="dialogFormVisible" center>
          <el-form ref="editForm" :model="dataForm" :rules="rules2" label-width="80px" size="small" label-position="right">

            <el-form-item label="用户名" :label-width="formLabelWidth" prop="username" >
              <el-input v-model.trim="dataForm.username" auto-complete="off" placeholder="请输入用户名" />
            </el-form-item>

            <el-form-item label="姓名" :label-width="formLabelWidth" prop="name">
              <el-input v-model.trim="dataForm.name" auto-complete="off" placeholder="请输入姓名" />
            </el-form-item>

            <el-form-item label="手机" :label-width="formLabelWidth" prop="phone">
              <el-input v-model.trim="dataForm.phone" auto-complete="off" placeholder="请输入手机" />
            </el-form-item>

            <el-form-item label="部门" :label-width="formLabelWidth" prop="deptName" required>
              <popup-tree-input
                :disabled="dataForm.status =='1' "
                :data="deptData"
                :props="deptTreeProps"
                :prop="dataForm.deptName"
                :node-key="String(dataForm.deptId)"
                :current-change-handle="deptTreeCurrentChangeHandle"
              />
            </el-form-item>

            <el-form-item label="角色" prop="roleList" label-width="120px">
              <el-select v-model.trim="dataForm.roleList" :disabled="dataForm.status =='1' " multiple placeholder="请选择" style="width: 100%;">
                <el-option
                  v-for="item in roles"
                  :key="item.id"
                  :label="item.roleName"
            ...
            <el-form-item label="岗位" prop="jobType" label-width="120px">
              <el-select v-model.trim="dataForm.jobType" :disabled="dataForm.status =='1' " placeholder="请选择" style="width: 100%;">
                <el-option
            ...
            <el-form-item label="状态" prop="lockFlag" label-width="120px">
              <el-switch
                :disabled="dataForm.status =='1' "
                v-model.trim="dataForm.lockFlag"
                active-color="#13ce66"
...
export default {
    ...
    return {
      ...
      rules2: {
        username: [{ required: true, message: '请输入用户名', trigger: 'blur' }, {
          ...
          message: '以字母开头，长度在4-12之间， 只能包含字符、数字和下划线'
        }],
        name:[{ required: true, message: '请输入姓名', trigger: 'blur' }],
        deptName:[{validator:deptValidator,trigger:'change'}],
        phone: [{ required: true, message: '请输入手机号', trigger: 'blur' }, { validator: checkTel, trigger: 'change' }],
        jobType:[{ required: true, message: '岗位类型不能为空', trigger: 'blur' }],
  ...
  created() {
    ...
    this.findDeptTree()
    // 加载数据字典
    this.getDict('用户状态').then(res=>{
      this.statusList = this.dicts.sort((a,b)=>a.itemValue - b.itemValue)
    })
    this.getDict('岗位类型').then(res=>{
      this.jobTypes = this.dicts.sort((a,b)=>a.itemValue - b.itemValue)
    })
  },
  methods: {
    // 加载用户角色信息
    ...
    findDeptTree: function() {
      getDept().then((res) => {
        ...
        this.deptDataS = [ ...this.deptData ]
      })
    },
    handleDeptInput: function(val) {
      let deptData = [ ...this.deptData ]
      if(!val) return this.deptDataS = deptData
      function getRes(list){
        return list.reduce((res,item,index)=>{ 
          let newItem = { ...item }
          if(newItem.children && newItem.children.length>0) 
            newItem.children = getRes([ ...newItem.children ])
          if(newItem.name.includes(val) || newItem.children.length>0)
            return [ ...res,newItem ]
          return [...res]
        },[])
      }
      this.deptDataS = getRes(deptData)
    },
    // 部门菜单树选中
    ...
    submitForm: function() {
      this.$refs['editForm'].validate(passed => {
        if(passed){
          const userRoles = []
    ...
    jobTypeFilter(val){
      if(val){
        ...
        return jobTypes.find(item => val==item.itemValue).itemText
      } 
      return ''
    },
  },

      ...
      >
        ...
        <el-form-item v-if="dataForm.type !== 2" label="菜单图标">
          ...
          >
            ...
            <el-input slot="reference" v-model.trim="dataForm.icon" placeholder="点击选择图标" readonly>
              <svg-icon
                v-if="dataForm.icon"
                slot="prefix"
        ...
        <el-form-item :label="menuTypeList[dataForm.type] + '名称'" prop="name">
          <el-input v-model.trim="dataForm.name" :placeholder="menuTypeList[dataForm.type] + '名称'" />
        </el-form-item>
        ...
        <el-form-item v-if="dataForm.type !== 0" label="授权标识" prop="perms">
          <el-input v-model.trim="dataForm.perms" placeholder="如: sys:user:add, sys:user:edit, sys:user:delete" />
        </el-form-item>

        <el-form-item v-if="dataForm.type !== 2" label="内部菜单" prop="isFrame">
          <el-radio v-model.trim="dataForm.isFrame" label="true">是</el-radio>
          <el-radio v-model.trim="dataForm.isFrame" label="false">否</el-radio>
        </el-form-item>
        <el-form-item label="租户类型" prop="tenantType">
          <el-checkbox-group v-model.trim="dataForm.tenantType">
            <el-checkbox v-for="item of tenantTypes" :key="item.id" :label="item.itemValue">
              {{item.itemText}}
            </el-checkbox>
          </el-checkbox-group>
        </el-form-item>
        <el-form-item v-if="[0,1].includes(dataForm.type)" label="是否隐藏" prop="hidden">
          <el-radio v-model.trim="dataForm.hidden" :label="1">是</el-radio>
          <el-radio v-model.trim="dataForm.hidden" :label="0">否</el-radio>
        </el-form-item>
        <el-form-item v-if="dataForm.type === 1 && dataForm.isFrame === 'true'" label="菜单路由" prop="component">
          <el-row>
            <el-col :span="22">
              <el-input v-model.trim="dataForm.component" placeholder="路由组件" />
            </el-col>
        ...
        <el-form-item v-if="dataForm.type !== 2" label="菜单URL" prop="url">
          <el-input v-model.trim="dataForm.path" placeholder="菜单URL" />
        </el-form-item>
...
export default {
  ...
  data() {
    return {
      ...
      dataRule: {
        ...
        tenantType:[{ required: true, message: '所属租户类型不能为空', trigger: 'blur' }],
      },
  ...
  methods: {
    ...
    handleEdit: function(row) {
      ...
      this.dialogVisible = true
      const { menuId, type, name, parentId, parentName, isFrame, component, path, perms, sort, icon,tenantType, hidden } = row
      ...
      this.dataForm.tenantType = tenantType.split(',')
      this.dataForm.hidden = Number(hidden)
    },
    // 菜单树选中
    ...
    submitForm: function() {
      ...
      newDataForm.tenantType = newDataForm.tenantType.join(',')
      if (!this.isEditForm) {
        saveMenu(newDataForm).then(response => {
          if (response.data.code === 200) {
            this.$message({
              type: 'success',
              message: '操作成功'
        ...
        updateMenu(newDataForm).then(response => {
          if (response.data.code === 200) {
            this.$message({
              type: 'success',
              message: '操作成功'
